...
## The gm / player / table boundary: take a position vs. referee

Three engine roles, three single responsibilities, deliberately *not* collapsed into one "run the whole table" skill. The `gm` is the **neutral referee**: it owns mechanics, dice, and canonical state, and is constitutionally forbidden from favoring a seat or making its choices. The `player` is the referee's **strategic opposite**: its entire job is the one thing the referee may not do — **take a position** — given a seat's turn brief, the visible state table, and a persona, returning exactly one Turn Report declaration line plus one line of table talk. The `table` (still unbuilt) is the **composition root**: the only role that knows the table is heterogeneous (which seats are human, which are AI and with which persona, who GMs), the one the user invokes, and the one that routes each decision to its source and feeds declarations to the GM — the GM is *called*, it never calls.

The split is not overhead, it is the thing that makes `playtest` possible. A GM that also plays every seat destroys strategy coverage (six seats become one mind wearing six hats — sample size one) and destroys referee neutrality (a GM holding a strategy can't be trusted to rule impartially) — and it fails *silently*, producing a plausible transcript with no real strategy diversity, the worst failure mode for a balance tool. Collapse is prevented structurally (the GM is handed declarations, never a seat's reasoning) and made detectable by an **attribution acceptance test** that holds on every harness: *every declaration in a transcript must trace to a single, distinct decision-source; a GM-authored declaration is a failed run, not a passed one.* This is what the golden transcript's "no GM ventriloquism" acceptance criterion already encoded.

The interface needs nothing invented: the unit of exchange across all three roles is the game's **Turn Report line** (a human types it, a `player` returns it, the GM consumes it identically), and the GM's existing per-seat **turn brief** is the input a `table` feeds to a `player`. Where a harness offers subagents, `table` will run each player decision in an isolated context so the GM never sees a seat's reasoning; on the disk-free / web baseline this degrades to soft isolation (one context, instructed hygiene + distinct attributable player calls) — consistent with the product's near-full-information stance. (Decision record: the M4-era creative exploration of this boundary, recoverable in git history at `memory-bank/active/creative/creative-gm-player-table-boundary.md`.)
