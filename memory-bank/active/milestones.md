...
- [ ] M4: Build the `gm` skill (mechanics applier, restated state table, turn-brief distillation, external-data hook resolution with fallback, transcript journal where disk exists), testable by a human playing all seats of Cannonball Rally — est. L3: complete feature with several coupled components
