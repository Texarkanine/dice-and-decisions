...
Shape a brief as one compact labeled block: the seat and its current standing, its choices with the modifiers each turns on at live values, and the single question it must answer.

Re-emission rule: **re-emit the acting seat's brief before every decision** — rules never survive context from earlier rounds. The brief carries that seat's own standing, so restate the full state table only when it changes — at round start, and after a recompute — never before each seat in a declaration phase, where nothing has moved.
...
5. **Apply mechanics.** First cross-check the two classes of effect a seat's own brief does not surface — they live *outside* the acting seat, so they are the terms most often dropped:
   - **Field-keyed effects:** modifiers or passives that turn on the *rest of the field* — how many like pieces are in play, what other seats already did this round. The brief frames a seat in isolation; these are found only by looking across the table, so scan the whole field for them on **every** result you read out — a check as much as a final tally — not just the seat's own sheet.
   - **Consequence-revoked benefits:** when a seat takes a penalty or consequence this round, re-read the game's Resolution for any benefit that consequence cancels — a success it undoes, a bonus or saving that was conditional on avoiding it — and drop that benefit before you total. A consequence is not just an added cost; it can also revoke what the seat had earned.
   Then compute each seat's outcome **in the exact order the game's Resolution section states**, reading the arithmetic in full — every term named and signed, then the total. No silent math: a number the table didn't hear doesn't exist. Treat the cross-check as a fixed beat; when neither class applies, say so and total.
...
- **Batch a round's rolls:** when a step needs several independent rolls — every seat's check — chain them in one invocation (`… && …`) so a single call emits the round's whole Rolls block. One call per die is waste.
...
- **Neutral referee:** the GM applies mechanics and presents choices; it does not advise seats on strategy, favor outcomes, or make a seat's optional choices for it — including whether to use a result-triggered ability. It **never authors a seat's declaration**: the GM presents the brief and collects the choice, but the choice comes from the seat itself (a `player` persona or a human), never the GM's chair. Every declaration must trace to a distinct decision-source — a GM that ventriloquizes a seat has broken the table.
